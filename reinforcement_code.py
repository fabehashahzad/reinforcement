...
charging_levels = ['80-90%', '60-70%', '<60%']
...
def calculate_reward(charging_level_idx, destination_idx):
    ...
    if charging_level_idx == 0 and destination_idx == 0:  # 80-90% -> Same Station
        ...
        return -1  # Penalty of -1 for other cases
...
while True:
    ...
    try:
        ...
        charging_level_idx = 0 if user_charging_level >= 80 else (1 if user_charging_level >= 60 else 2)
        destination_idx = destinations.index(predicted_destination)
        
        reward = calculate_reward(charging_level_idx, destination_idx)
        print(f"For charging level {user_charging_level}%, predicted destination: {predicted_destination}. Reward: {reward}")
    except ValueError:
